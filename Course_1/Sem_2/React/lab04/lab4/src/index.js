...
export class Calendar extends React.Component {
    ...
    changeDate = (value) => {
        this.setState({ date: value });//?изменение даты
    };
    selectDays = (day) => {
        for (let i = 0; i < this.state.selectedDays.length; i++) {
            if (
                ...
                return this.setState(({ selectedDays }) => ({
                    selectedDays: [
                        ...selectedDays.slice(0, i),//*метод slice() копирует ссылки на объекты в новый массив *// 
                        //... spread оператор расширения массива парамент "всё остальное"
