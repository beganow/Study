...
/*gethostname используется для получения имени хоста (hostname) текущей машины. Она принимает указатель на буфер, в который будет записано имя хоста, и размер этого буфера.
Функция возвращает 0 в случае успеха и -1 в случае ошибки.

gethostbyname используется для получения IP-адреса по имени хоста.
Она принимает имя хоста в качестве параметра и возвращает указатель на структуру hostent, которая содержит информацию о хосте, включая его IP-адреса. Если хост с таким именем не найден, то функция возвращает NULL.

gethostbyaddr используется для получения имени хоста по IP-адресу.
Она принимает указатель на структуру in_addr, содержащую IP-адрес хоста, размер этой структуры и семейство адресов (обычно AF_INET). 
Функция возвращает указатель на структуру hostent, которая содержит информацию о хосте, включая его имя. Если хост с таким адресом не найден, то функция возвращает NULL.*/