...
function dialog_window() {
    document.body.innerHTML +=
        ...
        "<div class='label' onclick='openDialog()'>Open</div>" +
        "<div class='header'>Chat with voice:</div>" +
        "<div class='history' id='history'></div>" +
        "<div class='question'><input id='Qdialog' placeholder='Введите вопрос'/> <br><button onclick='ask(\"Qdialog\")'>Ask</button></div>" +
        '</div>'
...
function openDialog() {
    if (dialogOn) {
        ...
        $('#dialog').animate({ 'margin-left': '-1300px' }, 1000, function () { })
        dialogOn = true
        clearInterval(timer)
    }
}
